...
\midrule
\textsc{d2s} (seed) & 65.6$\pm$8.1 & 67.0$\pm$6.6 & 47.3$\pm$13.1 & 44.7$\pm$10.8 \\
\textsc{d2s} (opt.) & \textbf{84.4$\pm$3.5} & \textbf{70.8$\pm$6.2} & \textbf{61.3$\pm$10.8} & \textbf{52.0$\pm$7.8} \\
\bottomrule
\end{tabular}
\end{table}
...
\textsc{data2skills} achieves 84.4\% on Breast Cancer with 8 rules vs. Decision Tree's 93.2\% with 16 rules. On Diabetes, \textsc{data2skills} achieves 70.8\% with 7 rules, matching Decision Tree's 69.0\% with 28 rules ($p=0.49$, paired t-test, not significantly different) while using $4\times$ fewer rules. On Wine and Iris, the statistical diagnoser underperforms due to multi-class threshold interactions---an LLM-powered diagnoser is expected to improve these results.
